	...
	: QDialog(parent, f)
{
    ...
    autolaod->setWhatsThis( tr("Load subtitle from OpenSubtitle Server automatically") );
...
void FindSubtitlesConfigDialog::setAutoLoad(bool b) {
	autoload->setChecked(b);
}

bool FindSubtitlesConfigDialog::autoLoad() {
	return 	autoload->isChecked();
}
